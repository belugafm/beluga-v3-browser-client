...
const containsOnlyEmojis = (text: string): boolean => {
    if (text.match(/[a-zA-Z0-9\u3040-\u309F]/)) {
        return false
    }
    const textGraphemes = new GraphemeSplitter().splitGraphemes(text)
    const m = text.matchAll(_emojiPattern)
    return textGraphemes.length == [...m].length
}
...
export const PlainTextComponent = ({ text, theme }: { text: string; theme: ThemeT }) => {
    ...
    const elements = multilineComponents.map((block, k) => {
        if (block.indexOf("```") == 0) {
            return <CodeComponent text={block} theme={theme} key={k} />
        } else {
